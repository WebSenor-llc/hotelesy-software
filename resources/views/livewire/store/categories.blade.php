<div>
    <div class="flex items-center justify-between mb-1">
        <h1 class="text-2xl font-bold text-slate-900">Store categories</h1>
        <div class="flex items-center gap-3">
            <a href="{{ route('store.index') }}" class="text-sm text-slate-600">← Store</a>
    ...
    <p class="text-sm text-slate-600 mb-6">Inventory categories for raw materials, beverages, liquor, housekeeping, engineering, stationery.</p>
        ...
        <table class="w-full text-sm">
            ...
            <tbody class="divide-y divide-slate-100">
                ...
                <tr>
                    ...
                    <td class="px-4 py-2.5 text-xs uppercase">{{ str_replace('_',' ', $c->type) }}</td>
                ...
                @empty
                <tr><td colspan="4" class="px-5 py-12 text-center text-sm text-slate-500">No categories.</td></tr>
                @endforelse
            </tbody>
        </table>
    ...
    @if($showForm)
        <form wire:submit.prevent="save" class="bg-white rounded-xl border p-6 grid md:grid-cols-3 gap-4">
            <div class="md:col-span-3 flex items-center justify-between">
                <h2 class="font-semibold">{{ $editId ? 'Edit' : 'New' }} store category</h2>
